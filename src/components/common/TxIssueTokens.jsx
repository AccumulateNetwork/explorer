...
const TxIssueTokens = (props) => {
  ...
  if (tx.data && tx.data.amount && tx.data.recipient) {
    if (tx?.message?.transaction?.body && !tx?.message?.transaction?.body?.to)
      tx.message.transaction.body.to = [];
    tx.message.transaction.body.to.push({
      amount: tx.data.amount,
      url: tx.data.recipient,
    });
    delete tx.data.amount;
    delete tx.data.recipient;
  } else if (
    tx?.message?.transaction?.body?.to?.length > 0 &&
    !tx?.data?.recipient
  ) {
    tx.amount = tx?.message?.transaction?.body?.to.reduce(
        ...
        <Descriptions bordered column={1} size="middle">
          ...
          {tx.amount > 0 && token ? (
            <Descriptions.Item label={'Amount'}>
              <Text>
                {tokenAmount(tx.amount, token.precision, token.symbol)}
              </Text>
              <br />
          ...
          {tx?.message?.transaction?.body?.to ? (
            <Descriptions.Item label={'To'} className={'align-top'}>
              ...
              {!error && !token && <Skeleton active title={false} />}
            </Descriptions.Item>
          ) : null}
        </Descriptions>
      ) : (
