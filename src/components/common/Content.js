...
const Content = props => {
  ...
  return (
    ...
    <Tabs defaultActiveKey={defaultType} activeKey={type} className="entry-content" renderTabBar={renderTabBar} style={{marginTop: 15}}>
        <TabPane key="ASCII">
            <div className="content break-all"><Text copyable>{textRaw}</Text></div>
        </TabPane>
        <TabPane key="JSON">
            {!jsonDisabled ? ( <SyntaxHighlighter language="json">{JSON.stringify(JSON.parse(textRaw), null, 4)}</SyntaxHighlighter> ) : null}
        </TabPane>
        <TabPane key="Base64">
            <div className="content code break-all"><Text copyable>{textBase64}</Text></div>
        </TabPane>
        <TabPane key="Hex">
            <div className="content code break-all"><Text copyable>{textHex}</Text></div>
        </TabPane>
    </Tabs>
    </div>
  );
