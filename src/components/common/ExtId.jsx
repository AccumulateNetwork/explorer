...
const ExtId = props => {
  ...
  const [currentShort, setCurrentShort] = useState(current.length > 32 ? current.substring(0,32)+"…" : current);
  ...
  const handleClick = (type, defaultType) => {
    if (defaultType === "ASCII") {
      switch (type) {
        case 'ASCII':
          setType('Base64');
          setCurrentShort(textBase64.length > 32 ? textBase64.substring(0,32)+"…" : textBase64);
          break;
        case 'Base64':
          setType('Hex');
          setCurrentShort(textHex.length > 32 ? textHex.substring(0,32)+"…" : textHex);
          break;
        case 'Hex':
          setType('ASCII');
          setCurrentShort(textRaw.length > 32 ? textRaw.substring(0,32)+"…" : textRaw);
          break;
        default:
          break;
      }
  ...
  return (
    <Input.Group compact className={"extid extid-"+type+" "+cssClass}>
      ...
      <Text className="extid-type" onClick={() => handleClick(type, defaultType)}>
        {type}
      </Text>
      }
      {(props.children.length > 0) ? (
        <Text className="extid-text" copyable={!props.compact ? "copyable" : null}>
        {!props.compact ? (
          current
        ) :
          currentShort
        ...
        <Text className="extid-text extid-empty" disabled>Empty</Text>
      }
    </Input.Group>
  );
